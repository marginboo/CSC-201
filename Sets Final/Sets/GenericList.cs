...
namespace Sets
{
    ...
    /// Additional changes made by Alden Boby
    /// </remarks>
    ...
    { private class ListNode
        ...
        { if (position < 0)
            ...
            DuplicateCheck(item);//Check items in list for duplicates
            node.data = item;
            ListNode curr = first,
                     prev = null;
        ...
        /// <summary>
        /// Check if the new item being added is already in the set
        /// </summary>
        /// <param name="item"></param>
        public void DuplicateCheck(T item)
        {
            int i = 0;
            ListNode curr = first;
            for (i = 0; curr != null && !curr.data.Equals(item); i++, curr = curr.next) ; // Search for item in GenericList
            if (curr == null)
            {
                //Do nothing item isn't present therefore can be added
            }
            else
            {
                throw new Exception("Item is already in the list");//Duplicate was found
            }

        }

        /// <summary>
        /// Similar to the position method except returns a boolean value instead of the index of the found value
        /// </summary>
        /// <param name="item"></param>
        /// <returns></returns>
        public bool FindItem(T item)
        {
            ...
            if (curr == null)
            {
                Console.WriteLine(string.Format("{0} not found", item));// item was not found
                return false;
            }   
            else
            {
                Console.WriteLine(string.Format("{0} found at index {1}", item, k));
                return true;
            }
        } //Find Item
